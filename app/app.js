...
var config = {
  ...
  app_name: '★Royal Three★',
  recaptcha_sitekey: '6LcQDhoTAAAAALfLvcuUUjJUKVeXbiHN5dGb_u8w',
  redirect_uri: 'https://royalthree.github.io',
...
//2016-01-19T05:18:12.550Z
//helpers.getDateAndTime = function(dateJson){
//  var d = new Date();
//  return d.toJSON();
//}
...
helpers.WinProbtoMultiplyer = function(winProb) {
...
if (worldStore.state.coin_type === 'BITS')
  {
  //(worldStore.state.user.balance / 100).toFixed(2).toString()
...
helpers.roleToLabelElement = function(user) {
  switch(user.role) {
    ...
    default:
      if ((user.uname == 'RTBot')||(user.uname == 'RTBot'))
        {
        return el.span({className: 'label label-primary'}, 'Bot');
        }
      else {
...
var MoneyPot = (function() {
  ...
  o.listUserBets = function(callbacks) {
    var endpoint = '/list-bets';
    makeMPRequest('GET', undefined, endpoint, callbacks, {
      data: {
  ...
  o.getuserstats = function(callbacks) {
...
var chatStore = new Store('chat', {
  ...
  Dispatcher.registerCallback('INIT_CHAT', function(data) {
    ...
    if (self.state.chatinit == false){
      self.state.chatinit = true;
    ...
    if (!error){
      ...
      MoneyPot.tip(params, {
          success: function(tip) {
              ...
              //alert("Successfully sent "+tipres[2]+"BTC to "+tipto);
    ...
    if ((!error) && (tipamount > 99)){
      ...
      MoneyPot.tip(params, {
          success: function(tip) {
              Dispatcher.sendAction('UPDATE_USER', { balance: worldStore.state.user.balance - tipamount});
              //alert("Successfully sent "+tipres[2]+"BTC to "+tipto);
              console.log('Successfully rained tip: '+(tipamount / 100).toString()+' to '+tipto+' BITS');
              self.state.sent_to++;
          },
          error: function(xhr) {
            console.log('Error' + tipto + '|' + tipamount + '');
          ...
          complete: function() {
            if (self.state.sent_to < (Object.keys(chatStore.state.userList).length - 1)){
              //tipto = tiplist[sent_to];
              Dispatcher.sendAction('SEND_RAIN',text);
            }
            else{

              text = "Rained a total of: "+tipres[1] + " " + coin_type_send + " to " + self.state.sent_to + " users";
  ...
  Dispatcher.registerCallback('PARSE_COMMAND',function(text){
    ...
    if (text.substring(0, 4) == "/tip") {
      ...
      Dispatcher.sendAction('NEW_MESSAGE', {channel: "lobby", text:text, created_at: d.toJSON(),user:{role: "BOT", uname: "HELPER"} });
...
var betStore = new Store('bet', {
  ...
  pay_tables: {
    ...
    'ROW1': [2,2,2,2,2,2,2,2,0.94,0,0,0,0,0,0,0,0],              //Player1 meow 1
    ...
    'ROW4': [500,50,50,2,1.2,0.12,1.1,1.1,0,1.1,1,0.12,1.2,2.1,50,50,500],       //Libby515    Lucky Strike
    //'ROW4': [121, 47, 13, 5, 3, 1.4, 1, 0.5, 0.3, 0.5, 1, 1.4, 3, 5, 13, 47, 121],
    //'ROW5': [999, 0, 50, 1, 1.2, 0, 1.1, 1.1, 0, 1.1, 1.1, 0, 1.2, 1, 50, 0, 999]
    'ROW5': [9999, 9, 8, 7, 6, 0.6, 0.3, 0.1, 0.05, 0.1, 0.3, 0.6, 6, 7, 8, 9, 9999]   //Vuiltje Blazing Better
  },
  ...
  Dispatcher.registerCallback('INC_HOUSE_EDGE', function(){
    if (self.state.house_edge < 0.05)
        self.state.house_edge += 0.001;
        Dispatcher.sendAction('UPDATE_BANKROLL');
    self.emitter.emit('change', self.state);
  });

  Dispatcher.registerCallback('MIN_HOUSE_EDGE', function(){
    self.state.house_edge = 0.0002;
    Dispatcher.sendAction('UPDATE_BANKROLL');
    self.emitter.emit('change', self.state);
  });
...
var worldStore = new Store('world', {
  ...
  /////////////////////////////////////////////////////////////////////
//  Dispatcher.registerCallback('SET_NEW_BET_HISTORY', function() {
//    self.state.newbetdata = true;
...
Dispatcher.registerCallback('SPAWN_PUCK', function(data) {
  ...
  Dispatcher.registerCallback('NEW_ALL_BET', function(bet) {
    ...
    else if ((bet.uname != worldStore.state.filteruser.str) && (worldStore.state.filteruser.str != '') && (worldStore.state.filteruser.str != 'User'))
        {
        return;
        }
    else{
  ...
  //      "invested": 10000.32
      ...
      MoneyPot.getBankroll({
          success: function(bankroll) {
            console.log('Successfully retrived bankroll data', bankroll);
...
//          "betted_profit": 41472.6666666667
          ...
          //self.emitter.emit('change', self.state);
          MoneyPot.getuserstats({
              success: function(ustats) {
                  console.log('Successfully retrived user stats from endpoint', ustats);
                  self.state.user.betted_count = ustats.betted_count;
                  self.state.user.betted_wager = ustats.betted_wager;
                  self.state.user.betted_profit = ustats.betted_profit;
                  self.emitter.emit('change', self.state);
              },
              error: function(xhr) {
                console.log('Error Userstats');
                if (xhr.responseJSON && xhr.responseJSON) {
                  alert(xhr.responseJSON.error);
                } else {
                  alert('Internal Error');
    ...
    if (worldStore.state.isLoading) {
      ...
      innerNode = el.div(
        ...
        el.span(
          ...
          helpers.convNumtoStr(nav_balance) + worldStore.state.coin_type,
          !worldStore.state.user.unconfirmed_balance ?
           '' :
           el.span(
             {style: { color: '#e67e22'}},
             //' + ' + (worldStore.state.user.unconfirmed_balance / 100).toFixed(2).toString() + worldStore.state.coin_type +' pending'
           ...
           el.div(
             ...
             el.div(
               ...
               el.span(null,
                 el.button(
                 {
                   className: 'btn btn-xs btn-primary',// + chatStore.state.newmsg == true ? 'btn-primary':'btn-default',
               ...
               el.span(null, '  Type /help for commands'),
...
var AutobetMultipler = React.createClass({
  ...
  _validateMultiplier: function(newStr) {
    ...
    } else if (helpers.getPrecision(num) > 3) {
  ...
  //AutobetStore.state.on_loss_mul  TOGGLE_ON_LOSS_MUL
...
render: function(){
  ...
  if (worldStore.state.isLoading) {
    ...
    // If user is logged in, let them submit bet
      if (AutobetStore.state.Run_Autobet){
    //    setTimeout(function(){ this._AutobetRoutine();},200);
        innerNode =
        el.button(
              {
                id: 'Stop-Auto-Bet',
...
var BetBoxLastBet = React.createClass({
  ...
  render: function() {
  //  var last_bet = worldStore.state.bets;
 var last_bet = '';
 var last_wager = helpers.convNumtoStr(100);
 var last_cond = '>';
 var last_target = 49.50;
 var last_outcome = 50.00;
 var last_profit = helpers.convNumtoStr(100);
 last_bet = '';
 //last_bet = worldStore.state.bets.toArray().map(function(bet){ return (bet.bet_id||bet.id);});
 if (worldStore.state.first_bet){
   ...
   last_wager = helpers.convNumtoStr(worldStore.state.bets.data[worldStore.state.bets.end].wager);//helpers.round10(worldStore.state.bets.data[worldStore.state.bets.end].wager/100, -2);
   last_cond = worldStore.state.bets.data[worldStore.state.bets.end].meta.cond;
   if (worldStore.state.bets.data[worldStore.state.bets.end].meta.kind == 'simple_dice')
   {
     last_target = worldStore.state.bets.data[worldStore.state.bets.end].meta.number.toFixed(2);
...
//   last_target = worldStore.state.bets.data[worldStore.state.bets.end].kind == 'simple_dice' ? worldStore.state.bets.data[worldStore.state.bets.end].meta.number.toFixed(2) : '-';
//   last_outcome = worldStore.state.bets.data[worldStore.state.bets.end].kind == 'simple_dice' ? worldStore.state.bets.data[worldStore.state.bets.end].outcome : '-';
  ...
  render: function() {
    return el.div(
      ...
      el.div({className: 'form-group', style: { marginTop: '-15px'}},
        //el.div({className: 'col-xs-10', style:{paddingRight: '2px',marginLeft:'-10px'}},
          el.div(
            ...
            el.span({className:'input-group-btn'},
              //{className:'col-xs-2', style:{paddingLeft: '2px'}},
              el.button(
                {
                  type: 'button',
...
var BetBoxTarget = React.createClass({
  ...
  render: function() {
    ...
    if (isError) {
      ...
      innerNode = el.span(
        {className: 'lead'},
        ((winProb * - 100)*-1).toFixed(2).toString() + '>n>' + (((winProb * -100) + 100 )-betStore.state.house_edge).toFixed(2).toString()
      );
    }
  ...
  _validateMultiplier: function(newStr) {
    ...
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      ...
      Dispatcher.sendAction('UPDATE_CHANCE_IN', {
        num: (winProb*100).toFixed(2),
        str: (winProb*100).toFixed(2).toString(),
        error: null
      });
      //' ' + (winProb * 100).toFixed(2).toString() + '%' //var winProb = helpers.multiplierToWinProb(betStore.state.multiplier.num);
...
var BetBoxChanceInput = React.createClass({
  ...
  _validateChance: function(newStr) {
    ...
    // Ensure str is a number
    if (isNaN(num) || !isFloatRegexp.test(newStr)) {
      Dispatcher.sendAction('UPDATE_CHANCE_IN', { error: 'CHANCE_MULTIPLIER' });
      // Ensure multiplier is >= 1.00x
    } else if (num < 0.01) {
      ...
      //todo  update multiplyer
      Dispatcher.sendAction('UPDATE_MULTIPLIER', {
        num: helpers.WinProbtoMultiplyer(num/100),
        str: helpers.WinProbtoMultiplyer(num/100).toFixed(2).toString(),
        error: null
      });
    }
  },
    ...
    return el.div(
      ...
      el.div(
        ...
        el.input(
          {
            type: 'text',
            value: betStore.state.ChanceInput.str, //'UPDATE_CHANCE_IN'
            className: 'form-control input-md',
            style: {fontWeight: 'bold'},
...
var BetBoxButton = React.createClass({
    ...
    return function(e) {
      ...
      MoneyPot.placeSimpleDiceBet(params, {
        ...
        if (AutobetStore.state.Run_Autobet){
          ...
          //  setTimeout(function(){}.bind(this), betStore.state.betVelocity);
        }
      });
    };
  },
  render: function() {
    ...
    return el.div(
      ...
      el.div(
        (AutobetStore.state.ShowAutobet|| AutobetStore.state.Run_Autobet) ? {className: 'col-md-4'}:{className: 'col-md-8'},
        innerNode
      )
    );
  }
      ...
      el.div(
        ...
        el.div(
          ...
          el.div(
            ...
            el.div(
              {className: 'col-xs-4 well well-sm', style: {marginTop: '-15'}},
              el.div(
                {className: 'col-xs-6'},
          ...
          // HR
            ...
            el.div(
              {style: {marginTop: '-15'}},
              el.div(
                {className: 'col-xs-2',style: {marginTop: '-15'}},
              ...
              el.div(
                {className: 'col-xs-3',style: {marginTop: '-15'}},
...
var PlinkoBetButton = React.createClass({
  ...
  //      color: n,
  //      path: generatePath(),//['L','L','L','L','L','L','L','L','L','L','L','L','L','L','R','L'],//
  //      wager_satoshis: 0,
  //      profit_satoshis: 0,
  //      isTest: true
  //    });
      ...
      MoneyPot.placePlinkoBet(params, {
        success: function(bet) {
          console.log('Successfully placed bet:', bet);
          // Append to bet list
              ...
              el.li(
                ...
                el.a({
                    ...
                    el.span({style:{color: '#ffffff',backgroundColor: config.hexColors.dark['ROW2']}},
                      //helpers.commafy(helpers.floor10(worldStore.state.bankrollbalance/100, -2))
            ...
            return el.div(
              ...
              el.p(
                ...
                el.span(
                  {
                    style: {
                      color: '#ffffff',//config.hexColors.dark[row],
  ...
  render: function (){
    ...
    return el.div(
      ...
      el.div(
        ...
        el.div ({className: 'rt'},
          el.div ({className: 'rt-field'},bet_table),
    ...
    MoneyPot.placeCustomBet(bodyParams, {
            success: function(bet) {
              // Update next bet hash
                ...
                console.log('successfully place roulette bet'+bet);
            },
            error: function(xhr) {
                disableChips = false;
                console.log('Error');
  ...
  render: function(){
    return el.div(
      ...
      el.div({className:'col-xs-4'},
        el.div({className:'lead col-xs-4',style: {fontWeight:'bold'}}, 'Chipsize '),
        el.div({className:'col-xs-8'},
          el.div({className: 'form-group'},
            el.div({className: 'btn-group btn-group-justified'},
              el.div({className: 'input-group btn-group'},
                el.input(
                    {
                      type: 'text',
      ...
      el.div({className:'col-xs-2 text-right'},
        el.button(
          { id: 'RT-CLEAR',
            type: 'button',
...
var RouletteBox = React.createClass({
  ...
  render: function() {
    ...
    //    ),
        el.div(
          {className: 'panel-body'},
          React.createElement(RouletteBoard, null)//'Coming Soon'
        ),
        el.div(
...
var MyBetsTabContent = React.createClass({
  ...
  render: function() {
    ...
    //          if (bet.meta.cond){
    //            innernode = bet.meta.cond + ' ' + bet.meta.number.toFixed(2);
    //          }else if (bet.cond){
    //            innernode = bet.cond + ' ' bet.target.toFixed(2);
    //          }
            /////////////
            var type;
              if (bet.meta.kind == 'simple_dice')
                {
                  type = 'simple_dice';
                }
            ...
            return el.tr(
              ...
              el.td(
                null,
                bet.meta.kind == 'simple_dice' ? bet.meta.cond + ' ' + bet.meta.number.toFixed(2) : type
              ),
              // roll
              el.td(
                null,
                bet.meta.kind == 'simple_dice' ? bet.outcome + ' ' : '-',
                bet.meta.isFair ?
                  el.span(
...
var FilterUserInput = React.createClass({
  ...
  render: function(){
    return el.div(
      {className: 'col-xs-4',style:{marginBottom:'-5px'}},
      el.div(
        {className: 'form-group col-xs-8'},//style: { marginTop: '-15px' }},
...
var FilterWagerInput = React.createClass({
  ...
  render: function(){
    return el.div(
      {className: 'col-xs-4',style:{marginBottom:'-5px'}},
      el.div(
        {className: 'form-group col-xs-10'},//style: { marginTop: '-15px' }},
...
var AllBetsTabContent = React.createClass({
  ...
  _onStoreChange: function() {
        if (allbetdelay == false)
          {
          allbetdelay = true;
          setTimeout(function(){allbetdelay = false; renderallbet = true;}, 250)
          }
        if (renderallbet == true)
            {
            renderallbet = false;
            this.forceUpdate();
            }
  ...
  render: function() {
    return el.div(
      ...
      el.div(
        ...
        el.div(
          {className: 'panel-body'},
          el.span({className: 'h6 col-xs-2'},'Filter New Bets By:'),
          el.div(
            {className:'well well-sm col-xs-10',style:{marginBottom:'-5px',marginTop:'-10'}},
            ...
            return el.tr(
             { key: bet.bet_id || bet.id},
              ...
              el.td(
                ...
                bet.kind == 'simple_dice' ? bet.cond + bet.target.toFixed(2) : bet.kind == 'custom' ? 'ROULETTE' : 'PLINKO'
              ),
              // Visual
              el.td(
                {
                  ...
                  el.div(
                    {
                      style: {
                        ...
                        left: 0,
                        top: 0,
                        width: bet.kind == 'simple_dice' ? bet.outcome.toString() + '%' : '99.99%',
                        borderRight: '3px solid #333',
                        height: '100%'
                      }
                    }
                  ...
                  el.div(
                    {
                      style: {
                        position: 'absolute',
                        top: 0,
                        left: bet.kind == 'simple_dice' ? (bet.outcome - 1).toString() + '%' : '98%'
                      }
                    },
                    el.div(
                      {
                      ...
                      el.span(
                        {style: {fontFamily: 'monospace'}},
                        '' + bet.kind == 'simple_dice' ? bet.outcome : ''
                      )
                    )
                  )
                )
...
function getuserbets(num){
  ...
  MoneyPot.listUserBets({
    success: function(bets) {
      var betsreversed = bets.reverse();
         ...
         el.div(
           ...
           'Royal Three'
            ...
            el.div(
              ...
              el.div({className:'well well-sm col-xs-12'},
              el.p(null, 'In order to play you will need a balance.  You can use the free faucet to try out some bets for free or you can fund your MoneyPot account.  You will need to sign-up for a free account with MoneyPot in order to play here.  After you have created an account you add the Royal Three casino app to you MoneyPot account.  There are two methods available to add funds at this point.  The more direct way is to deposit directly to your Royal Three account by clicking the deposit button at the top of the page and using the deposit address located there. The second is under your MoneyPot account page, you can find the deposit button to generate a new BTC deposit address.    Once your account is funded you can click on deposit from inside the app to bring coins over to play with.  Deposits are available to you after 1 confirmation.')
                )//end well
              ),
            el.div(
              {className:'col-xs-12'},
            ...
            el.div(
              ...
              el.span(null,
               ...
               el.span(null, ' or search google for many more.  Remember you can lose when playing and only risk what you are willing to lose. Royal Three is not responsible for mistaken bets or funds lost with MoneyPot.'
             )))))))
                )//end well
              ),
            el.div(
              ...
              el.div({className:'well well-sm col-xs-12'},
              el.p(null, 'If you wish to help someone out you can tip other users within Royal Three without having to withdraw your coins to Moneypot first.  Simply type /tip [username] [amount] [coin] (ex. /tip User 1000 BITS or /tip User 0.001 BTC) and the coins will be transferred instantly.  If the receiver wishes to see his balance update immediately they will have to click on the refresh balance button on top otherwise it will update automatically after 10 seconds. Optionally you may add "private" to the end of the tip and the tip will be sent silently in chat.  Please use caution when tipping users and do not loan coins to those you do not trust.'),
            ...
            el.div(
              {className:'col-xs-12'},
              el.span({className: 'h6 text-left'},'Support:'),
              el.div({className:'well well-sm col-xs-12'},
              el.p(null, 'If you need support contact at:'),
              el.p(null, 'royal3@gmx.com'),
  ...
  render: function() {
    switch(worldStore.state.currTab) {
      ...
      case 'MY_BETS':
        MoneyPot.listUserBets({
          success: function(bets) {
            console.log('[MoneyPot.listUserBets]:', bets);
            bets.map(function(bet){
              bet.meta = {
                cond: bet.kind == 'simple_dice' ? bet.cond : '<',
                number: bet.kind == 'simple_dice' ? bet.target : 99.99,
                hash: 0,//bet.hash,
              ...
              //if (bet.kind == 'simple_dice')
              //{
              //  bet.meta.kind = 'simple_dice';
              //}
              bet.meta.kind = bet.kind;
            })
            Dispatcher.sendAction('INIT_USER_BETS', bets.reverse());
          },
          error: function(err) {
            console.error('[MoneyPot.listUserBets] Error:', err);
          }
        });
...
$(document).on('keydown', function(e) {
  var H = 72, L = 76, C = 67, X = 88, A = 65, S = 83, D = 68, F = 70, G = 71,keyCode = e.which;
  ...
  if (keyCode !== H && keyCode !== L && keyCode !== X && keyCode !== C && keyCode !== A && keyCode !== S && keyCode !== D && keyCode !== F && keyCode !== G) {
    return;
  }
  ...
  switch(keyCode) {
    ...
    //  var downWager = Math.floor(betStore.state.wager.num / 2);
      var downWager = (betStore.state.wager.num / 2).toFixed(2);
      Dispatcher.sendAction('UPDATE_WAGER', {
        num: downWager,
